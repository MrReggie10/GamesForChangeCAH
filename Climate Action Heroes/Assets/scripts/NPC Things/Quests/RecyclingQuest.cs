...
public class RecyclingQuest : QuestType
{
    ...
    public override void QuestStart(GameObject npc, GameObject uiPanel, IShopCustomer shopCustomer)
    {
        ...
        uiPanel.GetComponent<QuestUIprefab>().SetName("Give 2 iron sheets to the recycling plant owner");
    ...
    void Update()
    {
        ...
        {
            if (item.itemType == Item.ItemType.iron_sheet)
            {
    ...
    public override void EndQuest()
    {
        ...
        {
            if (shopCustomer.GetInventorySystem().getItemList()[i].itemType == Item.ItemType.iron_sheet)
            {
                for (int j = 0; j < 2; j++)
                {
